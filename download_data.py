...
parser.add_argument('--tile_resolution', action='store', type=str, dest='tile_resolution',
                    default="16km",
                    help='The zoom resolution of the tiles. Must be coarser than the image resolution of layer.  Default:  16km')

# Multithreading options
parser.add_argument('--num_threads', action='store', type=int, dest='num_threads',
                    default=1,
                    help='Number of concurrent threads to launch to download images.  Default:  1')
...
def get_date_range(start_date_str, end_date_str):
    ...
    if start_date_str is None:
        ...
        if datetime.strptime(start_date_str, "%Y-%m-%d") < datetime.strptime(layer.date_min, "%Y-%m-%d"):
            print("No layer data from {}. Will begin from {} instead".format(datetime.strptime(start_date_str, "%Y-%m-%d"),
...
if tile_resolution not in ["32km", "16km", "8km", "4km", "2km", "1km", "500m", "250m", "31.25m"]:
...
for single_date in daterange(start_date, end_date):
    ...
    # Option 2: Pull the entire world as a single image
        ...
        cmd_list = ["gdal_translate", "-of", "--config", "GDAL_PAM_ENABLED", "NO", layer.format, "-co", "WORLDFILE=YES", "-projwin", "-180", "90", "180", "-90", infile, outfile]
